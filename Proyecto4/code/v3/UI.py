...
def trainingThread(pWinClass):
        while pWinClass.exitThread == False:
            if pWinClass.trainBtnPress:
                ...
                pWinClass.btnStop.grid(row=0, column=1)
                pWinClass.errorLabel.grid(row=1, column=0)
                pWinClass.networkErrorData = pWinClass.network.trainNetwork(
                    pWinClass.inputs[0], 
                    pWinClass.inputs[1],
                    pWinClass.inputs[2],
                    pWinClass.updateError)
                ...
                pWinClass.btnStop.grid_forget()
                pWinClass.errorLabel.grid_forget()
...
class TrainingWindow:
    def __init__(self, pParentWin, pNetwork):
        ...
        self.win.minsize(width=320, height=280)
    ...
    def trainNetwork(self):
        ...
        lr = self.entryLR.get()
        epsilon = self.entryEps.get()

        testSets = re.fullmatch('^[0-9]+$',sets)
        testLR = re.fullmatch('(^[0-9]+$|^[0-9]+\.*[0-9]+$)',lr)
        testEps = re.fullmatch('(^[0-9]+$|^[0-9]+\.*[0-9]+$)',epsilon)

        if (testSets and testLR and testEps):
            self.inputs = [int(sets), float(lr), float(epsilon)]
            self.trainBtnPress = True


    def stopNetworkTraining(self):
        self.network.stopTraining = True
    ...
    def updateError(self, pError):
        error = "Error: " + str(round(pError, 6))
        self.errorStr.set(error)

    def createUIElements(self):
        #Create frames
        self.titleFrame = Frame(self.mainFrame, height=50)
        self.titleFrame.grid(row=0, column=0, padx=2, pady=2, sticky=W+E+N+S)
        self.titleFrame.rowconfigure((0,1), weight=1)
        ...
        self.inputFrame.rowconfigure(tuple(range(3)), weight=1)
        ...
        self.btnFrame.columnconfigure((0,1), weight=1)
        ...
        self.errorStr = StringVar()
        self.errorStr.set("Error: 0.0")
        self.errorLabel = Label(self.titleFrame, textvariable=self.errorStr)
        ...
        label = Label(self.inputFrame, text="Learning rate: ")
        label.grid(row=1, column=0, padx=2, pady=2)

        self.entryLR = Entry(self.inputFrame)
        self.entryLR.grid(row=1, column=1, padx=2, pady=2)

        label = Label(self.inputFrame, text="Epsilon: ")
        label.grid(row=2, column=0, padx=2, pady=2)

        self.entryEps = Entry(self.inputFrame)
        self.entryEps.grid(row=2, column=1, padx=2, pady=2)
        ...
        self.btnStop = Button(self.btnFrame, text="Stop", command=partial(self.stopNetworkTraining))
        self.btnStop.grid(row=0, column=1)
        self.btnStop.grid_forget()
